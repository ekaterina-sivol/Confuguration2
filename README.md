...
* parse_package_name(package_name) - парсит имя пакета на groupId и artifactId.Параметры: package_name (str) - имя пакета в формате "groupId:artifactId".Возвращает: tuple - (group_id, artifact_id).
* build_pom_url(group_id, artifact_id, version, repository_url) - строит URL для POM-файла по Maven-конвенции.Параметры: group_id (str), artifact_id (str), version (str), repository_url (str).Возвращает: str - URL POM-файла.
* parse_dependencies_from_pom(pom_content) - парсит зависимости из содержимого POM-файла.Параметры: pom_content (str) - содержимое POM-файла.Возвращает: list - список зависимостей.
...
* should_filter_package(package_name, package_filter) - проверяет, нужно ли фильтровать пакет.
...
* process_cycles() - обрабатывает циклические зависимости для специального отображения.
...
### Общее описание.
#### Цель: выполнены дополнительные операции над графом зависимостей.
#### Требования:
1. (только для этого этапа) Поддержали режим вывода на экран порядка
загрузки зависимостей для заданного пакета. Сравнили результаты с
реальным менеджером пакетов. Если есть расхождения в результатах,
объяснили их наличие.
2. Продемонстрировали функциональность этого этапа на различных случаях
работы с тестовым репозиторием.
...
Результат работы программы со сложной структурой графа зависимости
<img width="950" height="1101" alt="image" src="https://github.com/user-attachments/assets/ea4aec88-f70e-44be-a31e-001726d5e2c8" />


Результат работы программы с алмазной структурой графа зависимости
<img width="942" height="1043" alt="image" src="https://github.com/user-attachments/assets/99f3b6dd-69bd-43b8-b973-2df9b9054d7c" />


Результат работы программы с циклической структурой графа зависимости
<img width="909" height="1043" alt="image" src="https://github.com/user-attachments/assets/a77885e4-61f4-4f35-9f5e-981b4f920e0f" />
